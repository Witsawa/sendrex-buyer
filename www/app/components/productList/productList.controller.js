class ProductListController {
  constructor($scope,Product,$ionicLoading,$state,ProductCategory,Shop) {
    ...
    if(!this.shopId) {
      ...
      Shop.shopProductCategories({id:this.shopId}).$promise.then((categories)=>{
        self.categories = self.categories.concat(categories)
      },()=>{
        //TODO error handling from product category
      })
    }
    this.category = 0;
    this.products = [];
  ...
  fetchProduct(onSuccess,onError,onFinally){
    ...
    if(this.category!=0){
      if(this.shopId) {
        filter.filter.where['shopProductCategoryId'] = this.category
      }else{
...
ProductListController.$inject = ['$scope','Product','$ionicLoading','$state','ProductCategory','Shop']
